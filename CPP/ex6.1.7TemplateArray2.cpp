...
//	}
   ...
   //   Array размера size, заполненный значениями
    ...
    Array(size_t size = 0, const T& value = T()) {
		 _array = (T*) (new char[size * sizeof(T)]);
		_size = size;
    ...
    Array(const Array & a) {
		_array = (T*) (new char[a._size * sizeof(T)]);
		_size = a._size;
		for(size_t i = 0; i < _size; i++) 
			new (_array + i) T(a._array[i]);
	}

    //   деструктор, если он вам необходим.
    ~Array() {
		operator delete[] ((char*)_array);
	}
